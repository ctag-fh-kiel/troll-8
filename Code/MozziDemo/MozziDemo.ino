...
void setup(){
  ...
  aSaw1.setFreq(262>>2); // set the frequency
  aSaw2.setFreq(278>>2); // set the frequency
  aSaw3.setFreq(294>>2); // set the frequency
  aSaw4.setFreq(311>>2); // set the frequency
  aSaw5.setFreq(330>>2); // set the frequency
  aSaw6.setFreq(349>>2); // set the frequency
  aSaw7.setFreq(370>>2); // set the frequency
  aSaw8.setFreq(392>>2); // set the frequency
  bSaw1.setFreq(415>>2); // set the frequency
  bSaw2.setFreq(440>>2); // set the frequency
  bSaw3.setFreq(466>>2); // set the frequency
  bSaw4.setFreq(494>>2); // set the frequency
  bSaw5.setFreq(523>>2); // set the frequency
  bSaw6.setFreq(554>>2); // set the frequency
  bSaw7.setFreq(587>>2); // set the frequency
  bSaw8.setFreq(622>>2); // set the frequency
...
void updateControl(){
  ...
  int cutoff_freq = (readADC(0)>>2) + kFilterMod.next()*(readADC(2)>>5);
  svf.setCutoffFreq(cutoff_freq);
  svf.setResonance(readADC(1)>>2);
  if(kMetro.ready()){
    byte val = ~(1 << (cnt%8));
    writeRegGPIO(MCP23S18_GPIOA, val);
    cnt++;
    kFilterMod.setFreq((float)rand(255)/64);  
  }
}
...
int updateAudio(){
  int s = 0;
  if(voiceOnOff1&0x01) s += aSaw1.next() >> 2;
